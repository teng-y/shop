<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="trade in tradeInfoList" :key="trade.id">
        <span
          class="username"
          @click="changeSelected(trade,tradeInfoList)"
          :class="trade.isDefault === '1' && 'selected'"
        >{{trade.consignee}}</span>
        <p>
          <span class="s1">{{trade.fullAddress}}</span>
          <span class="s2">{{trade.phoneNum}}</span>
          <span class="s3" v-show="trade.isDefault === '1'">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <textarea placeholder="建议留言前先与商家沟通确认" v-model="message" class="remarks-cont"></textarea>
      </div>
      <div class="line"></div>
      <div class="bill">
        <h5>发票信息：</h5>
...
export default {
  ...
  methods: {
    ...
    async getUserAddressList() {
      ...
      if (result.code === 200) {
        this.tradeInfoList = result.data;
      } else {
    ...
    //选择用户收货地址高亮
    changeSelected(trade, tradeInfoList) {
      tradeInfoList.forEach(item => (item.isDefault = "0"));
      trade.isDefault = "1";
    },
    //获取订单编号
    async submitOrder() {
      let tradeNo = this.tradeInfo.tradeNo; //准备交易编号
      let tradeInfo = {
      ...
      try {
        ...
        alert('请求提交订单失败'+ error.message)
      }
    }
  },
  computed: {
    defaultAddress() {
      return this.tradeInfoList.find(item => item.isDefault === "1") || {};
    }
  }
};
</script>
